...
\textbf{Sub-Questions:}
\begin{itemize}
    \item \emph{R1: \begin{highlight}How to enable co-creative dialogues between humans and AI, and how does it influence the roles and effectiveness of human-AI co-creativity?\end{highlight}}
    \item \emph{R2: Which interaction design principles can guide the development of effective co-creative systems?}
\end{itemize}
\end{quote}



I address these questions by synthesising findings from my research studies, emerging literature, and observations from practice. In Chapter 3, I established a framework for dialogic co-creativity comprising six core elements: bidirectional communication, shared collaborative space, iteration, mutual influence, mutual understanding, and contextual awareness. I also distinguished three levels of dialogic interaction: pseudo-dialogue, weak asymmetric dialogue, and full dialogue \cite{Bown2024-yx}. Throughout this thesis, I used this framework as an analytical lens to examine how dialogic elements influence roles, agency, and effectiveness in human-AI co-creativity.

Here, I propose that these three levels of dialogue correspond to three distinct types of co-creative AI systems, each requiring different dialogic elements to be prioritised. I reframe the elements of dialogic co-creativity as design principles and propose specific guidelines for their implementation at each level.

\subsection{Three Levels of Co-Creative Systems}

From the studies in the preceding chapters and a survey of the literature and practice, I propose there are three levels of co-creative systems. Each level prioritises different dialogic elements and requires distinct design approaches.

\textbf{Level 1: Generative co-creators} prioritise \textbf{mutual influence} and \textbf{iteration}. These systems, exemplified by text-to-image generators such as those used in Chapter 5, operate primarily as generators with minimal dialogic capabilities. They enable users to explore creative possibilities through generative variability and support basic iterative workflows. However, they lack conversational interfaces, shared collaborative spaces, and robust context awareness. Their role exists primarily in early stages of the creative workflow, serving to ideate and explore possibilities. 

As such, mutual influence is a desirable design principle for this level. The AI influences the user by stimulating novelty and shifts in perspective, facilitating new creative directions. On the other hand, the user can influence the system through expressive control, allowing them to steer it towards their creative intentions. This dynamic of mutual influence enables productive exploration. A key aspect of this exploration is iteration, allowing users to pursue interesting directions further. Hence, iteration is prioritised alongside mutual influence.

\textbf{Level 2: Communicative and iterative co-creators} add \textbf{mutual understanding} and \textbf{bidirectional communication} to the elements prioritised at Level 1. These systems, such as chatbots like ChatGPT or Vorges in chat-only mode, enable conversational interaction and can iterate on outputs when the creation is in language. They support alignment of goals and intentions through dialogue, but remain limited in supporting interaction through the creation itself.

Enabling conversational interaction, or bidirectional communication, may appear trivial. However, such types of interaction were not commonplace at the beginning of this thesis, before the emergence of conversational large language models. Early explorations in this thesis examined capabilities of earlier models to engage in these types of interactions and, through early experiments such as the one in Chapter 3, argued for the value of this type of interaction in enabling human-AI co-creativity.

However, throughout this thesis, as conversation emerged as the dominant design pattern with generative system, I also argued that conversational interaction alone is not enough, particularly if it only enables a client-producer dynamic where interaction is limited to instruction following. A dynamic of mutual understanding is important, where the system asks for clarification, suggests alternative viewpoints and possibilities, and seeks to form an understanding of the user. Similarly, allowing the user to query the system for explanations and clarification, as well as enabling discussion on goals, intentions, and preferences, distinguishes these systems from purely generative ones. As such, mutual understanding is highlighted as a key design principle for this level of co-creativity. This is primarily enabled through conversational design, but also through visibility of creative affordances and primitives, enabling the user to form a shared understanding of the system. 

\textbf{Level 3: Collaborative, embedded and adaptive co-creators} integrate all elements from previous levels and additionally prioritise \textbf{shared collaborative space} and \textbf{context awareness}. These systems, such as Vorges with the Shared Space, enable a space where both human and AI can contribute to the creative product, allowing interaction not only about the creation, but through the creation itself. In Chapter 4, I showed the value of a shared collaborative space on perception of agency from the user and their involvement in a co-creative interaction. 

In order to be as valuable and relevant as possible, these systems can reference relevant context, for example, user style, previous work, and relevant files. Popular systems such as Cursor exemplify a co-creator with a shared collaborative space where the AI acts with awareness of relevant files, user work, and workspace. Ideally, at this level, the system is embedded in the user's main workspace, allowing for collaboration, sharing a common space, and context awareness, enabling it to act within relevant context. 

\subsection{Design Principles and Guidelines}

Each dialogic element can be operationalised through specific design principles and guidelines. Table \ref{tab:dialogic-principles} presents these principles organised by the three levels of co-creativity.

\begin{table}[htbp]
\centering
\caption{Design principles for dialogic co-creativity across three levels}
\label{tab:dialogic-principles}
\begin{tabularx}{\textwidth}{lX}
\toprule
\textbf{Element} & \textbf{Design Principles and Guidelines} \\
\midrule
\textbf{Mutual Influence} & \textbf{Serendipity:} Leverage generative variability to induce perspective shifts and adaptation of creative directions. \\
& \textbf{Expressive Control:} Enable multimodal inputs beyond text to allow users to express intentions by showing, not only telling. \\
\midrule
\textbf{Iteration} & \textbf{Consistency and Refinement:} Support iterative workflows that allow refinement while maintaining consistency across versions. \\
& \textbf{Stateful Histories and Journeys:} Provide rich histories and stateful user journeys that preserve exploration and enable backtracking. \\
\midrule
\textbf{Mutual Understanding} & \textbf{Personalisation:} Allow users to build shared understanding through training, fine-tuning, and style ownership. \\
& \textbf{Expose Creative Primitives:} Make visible what the system can and cannot do, enabling users to form accurate mental models. \\
\midrule
\textbf{Bidirectional Communication} & Enable two-way conversational exchanges that support clarification, feedback, and alignment of goals and intentions. \\
\midrule
\textbf{Shared Collaborative Space} & \textbf{Separate Through and About:} Provide dedicated spaces for collaboration through the creation, separate from conversational spaces. \\
& \textbf{Visibility of Contributions:} Clearly distinguish AI contributions through edit tracking and highlighting, enabling users to review, accept, or reject changes. \\
\midrule
\textbf{Context Awareness} & \textbf{Connect to External Sources:} Integrate awareness of environmental data, trends, and broader sociocultural context. \\
& \textbf{Existing Work:} Maintain awareness of the user's workspace, previous work, files, and established preferences. \\
\bottomrule
\end{tabularx}
\end{table}
...
This thesis argues that a central problem in human-AI co-creativity is that the prevailing modes of interaction with generative AI often diminish user creative agency through what I now term here as\textbf{severed creative agency}: a fundamental disconnect between a user's creative intentions and the actions performed by the AI. 

This severing emerges from a shift in role distribution induced by the technology and linear interaction design patterns. Humans increasingly operate within the \textit{intentional space}, where creative goals, visions, and high-level decisions reside, while AI systems take on roles in the \textit{action space}, where concrete artefact-level operations such as drawing, writing words, or playing notes occur. While assuming primarily intentional roles, severed creative agency arises for two reasons: First, a difficulty in steering generative AI. Second, from a lack of involvement at the action level.

Dialogic co-creativity is a promising approach to address this by more closely \textbf{aligning intention and action}, contributing to increased agency and effectiveness of interactions. Dialogic co-creativity involves designing iterative interactions that promote \textbf{mutual adaptation and understanding}, contributing to a better alignemnt between creative intentions and outputs. Co-creative dialogues, involving interaction not only \textit{about} the creative process (intentional level) but also \textit{through} the creative process itself (action level) can also contribute to the user itself making contributions to the creative output. 

As the core contribution of this thesis, \begin{highlight}first I developed a framework for dialogic co-creativity comprised on six elements, which are as follows:
...
These elements were derived from the literature on dialogue and human-computer interaction, and are discussed in detail in Chapter 3. Throughout the rest of the thesis, I used these elements as a navigational lens to investigate the impact of dialogic interaction on the roles, agency, and effectiveness of human-AI co-creativity.\end{highlight}

In Chapter 4, I found that having a share creative space in addition to a conversational interface can lead to more balanced user and AI roles, with more involvement by the user at the artefact level. 

In Chapter 6, I conducted a case study in the image generation, where I was main artist and creative practitioner, working closely with the creative team of a magazine. At the time of conducting that study, image generaiton systems lacked virtually every aspect of dialogic co-creativity. They did not have conversational capabilities, there was no share space to make shared contributions to the image, the models did not have robust editing capabilities that allowed for meaningful iteration. There was not formation of a mutual influence and mutual understanding between the user and the system, and the system did not generate with a broader context other than the prompt.

In other words, the system used was a mere generator, and the case study illumintated the limits of the interaction and the roles assumed through the lack of dialogic co-creativity elements. In that case study, I acted largely as a curator of outputs. The system acted largely as a mere generator, producing outputs that attempted to align with the prompt, but without any iterative interaction that allowed us to collaborate. While the outputs of the case study were printed and considered to fulfill the brief, subjectively as a practitioner, I felt that the interaction was not co-creative, but rather I used a generative photography tool. 

In Chapter 7, I conducted a case study in the field of new media and sound generation. In this case {...}

In this Chapter, I make a final contribution in the form of a set of design principles that operationalise dialogic co-creativity, offering concrete guidance for the development of co-creative AI systems.

These are framed within a distinction that I propose in three levels. From the studies in the preceeding chapters and a survey of the literature and practice, I propose there are three levels of co-creativity: 

Level 1 Co-creators - Generative: An example are image generation systems like the one used on Chapter 5. These systems have virtually no elements of dialogic co-creativity. From the types of dialogue, they are pseudo-dialogue, \cite{Bown2024-yx}. 

- Also, the systems used in Chapter 6. They enable a degree of context awarness, but lack other elemenents. 

- Lack all elements of dialogic co-creativity: bidirectional communication, shared collaborative space, iteration, mutual influence, mutual understanding, contextual awareness.

- However, they can produce novel outputs, by virtue of being generative, they can generate novel outputs that stimulate creativity, for example, a designer ideating, or rich creative practice, that open new possibilities. 

- They are limited at engaging in an iterative process of creative refinement. Limited collaboration. Feel closer to a tool. 


These systems are not necessarily undesirable, and indeed rich creative practices have been developed with pure generators \cite{smith}. Perhaps not all systems aim to reach a high level of co-creativity, either because the implementation is more complex, the underlying model does not have the necessary capabilities, or else. In this case however, the system can still aim to implement design principles that maximise their effectiveness given their constraints.

Level 2: communicative and iterative. 

These systems are those that are able to communicate with the user and in some case iterate on the output. An example of these systems is chatbots such as ChatGPT, or Vorges, in the chat-only version. These systems have the dialogic element of bidirectional communication, and iteration to some degree. In the case of chatbots iteration is possible if the the creation is in language, but it is harder when it involves other modalities. 

Level 3: collaborative, embedded and adaptive. 

Lastly, the highest level of co-creators, which integrate dialogic elements from below levels, but additionally, are more embedded and collaborative. And example is Vorges with the Shared Space, which shares a creative workspace with the user, or Cursor, which shares a creative workspace with the user and the codebase.


\section{Design Principles for Human-AI Co-Creativity}
...
Level 3: collaborative and embedded
...
Each of the levels of co-creativity above are valid and may have use cases. As such, different design principles and dialogic elements can be prioritised for each. 

As such, in this section, I frame the elements of dialogic co-creativity as design principles, and add 3 more that emerged from the literature and practice.

I propose that from the perspective of dialogic interaction design, there are three levels of co-creative interaction. 
...
\item Lean into the unique characteristics of the generative AI medium
\end{itemize}
\end{tcolorbox}


\begin{tcolorbox}[colback=purple!5,colframe=purple!75!black,title=\textbf{Principle 2: Personalisation}]
\textbf{Description:} Enable users to personalise and own their creative style through model training, fine-tuning and customisation.

\textbf{Guidelines:}
\begin{itemize}
\item Allow users to train or fine-tune models on their own datasets
\item Allow users to customise and own their style through
\end{itemize}
\end{tcolorbox}
...
\textbf{Guidelines:}
\begin{itemize}
\item Support iterative workflows that allow refinement while maintaining consistency
\item Provide rich histories and stateful user journeys that preserve exploration
\end{itemize}
\end{tcolorbox}
