...
\begin{quote}
    What are the elements that constitute a co-creative dialogue between humans and AI?
\end{quote}
To answer this question, I develop a comprehensive framework for Dialogic Co-Creative Artificial Intelligence. This framework consists of two complementary components, developed across two sections in this chapter, that together provide both practical guidance and theoretical grounding for understanding human-AI co-creativity.

\subsection{Typology of Actions for Dialogic Co-Creative Artificial Intelligence}

The first component presents a typology of actions for Dialogic Co-Creative Artificial Intelligence. These actions are organised into two categories: actions performed \textit{through} the artifact and actions performed \textit{about} the artifact.
...
\hline
\textbf{Actions through the artifact} & \textbf{Actions about the artifact} \\ \hline
\textbf{Create} \newline Propose an entirely new creation or artifact & \textbf{Set goal} \newline Establish or communicate the overarching creative intention \\ \hline
\textbf{Extend} \newline Build upon or continue an existing artifact & \textbf{Request} \newline Ask for specific actions or modifications \\ \hline
\textbf{Branch} \newline Generate multiple alternative extensions or variations & \textbf{Clarify} \newline Resolve ambiguity or uncertainty in requests \\ \hline
\textbf{Transform} \newline Modify an artifact's fundamental style or characteristics & \textbf{Acknowledge} \newline Confirm mutual understanding between participants \\ \hline
\textbf{Refine} \newline Enhance specific details or polish elements of an artifact & \\ \hline
\end{tabular}
\end{table}

This typology emerged from emerged from empirical observation and analysis presented in a paper at the GenAI and HCI workshop at CHI 2022. 

\subsection{Principles of Dialogic Co-Creativity}

The second component of the framework identifies six preliminary principles of dialogic co-creativity. While the action typology focuses on specific action affordances, these elements capture the broader design principles for Dialogic Co-Creative AI. I propose these principles need to be present when designing Dialogic Co-Creative AI. 
...
\hline
\textbf{Principles for Dialogic Co-Creative Artificial Intelligence}\\ \hline
...
This multi-disciplinary synthesis, discussed in the second Section of this Chapter, revealed recurring patterns across domains: effective co-creative dialogues requires communication channels, shared workspaces, reciprocal influence, mutual modeling, iterative refinement, and contextual grounding. These insights crystallised into the six preliminary principles presented in this chapter.

\begin{table}[htbp]
\centering
\caption{Literature Foundations for Dialogic Interaction Elements}
\label{tab:literature_sources}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|p{3cm}|p{4.5cm}|p{9cm}|}
\hline
\textbf{Element} & \textbf{Key Sources} & \textbf{Contribution} \\ \hline
Bidirectional Communication & 
\begin{tabular}[c]{@{}l@{}}
Hayes \& Reddy (1983)\\
Rezwana \& Maher (2022)\\
Ashktorab et al. (2021)\\
ChatGPT case study
\end{tabular} & 
\begin{tabular}[c]{@{}l@{}}
Established need for human-like dialogue\\
Showed bidirectional > one-way for engagement\\
Demonstrated impact on perceived intelligence\\
Illustrated mass appeal of conversational AI
\end{tabular} \\ \hline
...
\end{tabular} & 
\begin{tabular}[c]{@{}l@{}}
Theorised dialogic space for co-construction\\
Empirically validated shared workspace benefits\\
...
\end{tabular} & 
\begin{tabular}[c]{@{}l@{}}
Defined dialogue as reciprocal shaping\\
Historical model of collaborative discovery\\
Case study of perspective shifts in music\\
Value of Socratic method in conversational AI design
\end{tabular} \\ \hline
...
\end{tabular} & 
\begin{tabular}[c]{@{}l@{}}
Theorized shared meaning-making\\
Showed need for AI explainability\\
Framework for interpretable ML
\end{tabular} \\ \hline
...
\end{tabular} & 
\begin{tabular}[c]{@{}l@{}}
Creativity as reflective conversation\\
Practical value or iterative tools\\
Iterative loops improve performance\\
Iteration reveals richer insights
\end{tabular} \\ \hline
...
Bown (2021)\\
Cursor, Copilot, v0\\
Author's installations
\end{tabular} & 
\begin{tabular}[c]{@{}l@{}}
Meaning depends on situated context\\
Creativity embedded in environment\\
Integration with coding workspace\\
...
This framework was used as the analytical lens used for the remainder of this thesis, providing the conceptual tools to examine the remaining research questions:

\begin{itemize}
    \item R2: How do the presence or absence of the elements of co-creative dialogue influence the roles and agency of participants?
    \item R3: Which interaction design principles can guide the development of effective and human-centred Dialogic Co-Creative Artificial Intelligence?
\end{itemize}


\section{Part 1: A typology of actions for Dialogic Co-Creative Artificial Intelligence}

The following short-paper publication develops an initial typology of actiona. 
...
Dialogue has been widely discussed in the context of human-computer interaction, mixed-initiative systems and co-creative systems \cite{Allen1999-sr, Yannakakis2014-zs, Deterding2017-wh} as a means to model human-computer interaction, however, it is yet to be formalised as both a theoretical concept and an actionable concept for interaction design. This chapter builds upon that work and on previous work by \cite{Bown2020-oc, Bown2024-yx}. As a unique contribution, in this chapter I distill the elements that comprise dialogic interaction and how each can contribute to more effective human-AI co-creativity, thus providing a theoretical foundation for dialogic co-creativity as an interaction design concept. 
...
\subsubsection{A note on interaction vs action}: \begin{highlight}Throughout this thesis, I use the terms ``action'' and ``interaction'' almost interchangeably. As a response to reviewer comments, I provide the following nuanced distinction. When I use ``interaction,'' I refer more closely to an exchange between the AI agent and the human, where the AI is aware of and responds to the human's behaviour either through a conversational exchange or through a mutually visible action on a creation.  When I use ``action,'' I refer more closely to operations that act directly on an artefact, which may or may not involve mutual awareness between a human and AI. The reader may note that acting on an artefact through a computer is itself a type of interaction between the computer and the human. However, in the context of this thesis, where I investigate the AI's role as a collaborative agent, it is worth differentiating between operations that involve communication and mutual awareness between an AI agent and a human (interaction), and traditional human-computer operations (action). For example, if I play a note when co-creating a song with an AI agent, and the AI agent is aware of this action and can respond to it, then I usually refer to this as an interaction. By contrast, if I am working with a software DAW where I play a note that no AI agent is immediately aware of, then I refer to this as an action. 

In other words, an interaction is considered a type of action that involves mutual awareness and an implicit or explicit exchange of information between AI agent and human.\end{highlight}
...
In \textbf{pseudo-dialogue}, the interaction is linear. The user provides an instruction (for example, a prompt) and the computer generates. The user can evaluate the output and adjust the next prompt accordingly. The computer does not form a model of the user based on an iterative back-and-forth, nor there is a conversational interface. The system is simply a generator; it is the user who adjusts their mental model of how the system works based on repeated interaction, and iterates inputs accordingly.  An example of \textbf{pseudo-dialogue} can be found in \textbf{text-to-image systems}. 

By contrast, in \textbf{weak asymmetric dialogue} there is a conversational interaction, but the conversation (language) and creation (artifact) spaces are disjointed, limiting the co-creative potential of the interaction. The user still largely provided instructions for the generative system, who executes actions on the action space. An example of \textbf{weak asymmetric dialogue} can be found in interactions with \textbf{LLM-powered chatbots} moreover, the system does not fully model the user.
...
In the following sections, I expand upon this framing and investigate in more detail what components characterise \textbf{full dialogue }and how they can be enabled through interaction design. 
...
- In this thesis I set to explore dialogic co-creativity as an interaction design framework 
- Defining dialogic interaction, therefore is key
- Expanding on the above, and as a core contribution of this thesis, I propose a set of six elements that comprise dialogic interaction
- These are presented as general elements, that can then serve as guidelines fr interaction design
- The identificiation of these elements derives by drawing from the literature on human-computer interaction, as well as broader multidisciplinary literature on dialogue. 
- For clarity, the elements are presented below, and the follow sections discuss and justify their derivation. 
- The intention of this elements is to serve as a guideline to consider the level of dialogic interaction may present. 
- As a core contribution, expanding on the above I propose a set of elements of dialogic co-creativity:



    \begin{itemize}
        \item \begin{highlight}Bidirectional Communication: Two-way communicative or conversational exchanges.\end{highlight}
        \item \begin{highlight}A Shared Collaborative Space: There is a common collaborative environment separate from instructions or chat interfaces.\end{highlight}
        \item \begin{highlight}Mutual Influence: Each party's perspectives and goals evolve as a result of the interaction and each agent drives the other to new unexpected directions.\end{highlight}
        \item \begin{highlight}Mutual Understanding: Each agent actively seeks to understand the other's world-models, goals, perspectives, preferences, etc.\end{highlight}
        \item \begin{highlight}Iteration: The creative product and the shared meanings evolve iteratively.\end{highlight}
        \item \begin{highlight}Context-awareness: Actors bring in the relevant context and act within it.\end{highlight}
    \end{itemize}
    ...
    \caption{Dialogic co-creativity as an interaction design concept is comprised of six essential elements: bidirectional communication, a shared collaborative space, mutual influence, mutual understanding, iteration, and context-awareness}
    \label{fig:enter-label}
\end{figure}
...
\begin{highlight}As established earlier, dialogic co-creativity involves both interaction \textit{through} the artefact (implicit sense-making emerging from mutually visible actions) and interaction \textit{about} the artefact (explicit communication). These two modes of interaction require different enabling elements: \textit{bidirectional communication} is necessary for interaction about the artefact, enabling agents to explicitly exchange goals, intentions, and feedback. Conversely, a \textit{shared collaborative space} is necessary for interaction through the artefact, providing a common environment where agents can engage in mutually visible actions that facilitate implicit sense-making. Both elements are therefore essential for full dialogic co-creativity.\end{highlight}
...
\textbf{\begin{highlight}Shared Collaborative Space\end{highlight}} 
& \begin{highlight}In creative dialogues, having a common collaborative space is important (shared text editors, DAWs, Canvases). Co-creative AI's can benefit from separating communication from creation and enabling a dedicated space for co-creation where both AI and user can see and edit the creation.\end{highlight} \\ \hline
...
\textit{\begin{highlight}In dialogic co-creativity, humans and AI engage in an iterative interaction with bidirectional communication in a cycle of mutual influence and understanding, while co-creating in a shared collaborative space and with awareness of the relevant context.\end{highlight} }
