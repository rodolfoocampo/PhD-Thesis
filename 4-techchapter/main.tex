...
In \textbf{pseudo-dialogue}, the human provides instructions to the computer, which then generates an output for the human to evaluate.  An example of \textbf{pseudo-dialogue} can be found in \textbf{text-to-image systems}. The AI only partially and non-iteratively forms a model of the user's intention. 

By contrast, \textbf{weak asymmetric dialogue} allows for some degree of bidirectional communication, but the communication and action spaces are disjointed. Although the communication may flow both ways, the relationship largely reflects a client-producer dynamic. An example of \textbf{weak asymmetric dialogue} can be found in interactions with \textbf{LLM-powered chatbots} producing images by calling an external image generation tool, without an evaluation or perception of it (the AI is constrained to the communication space). 
